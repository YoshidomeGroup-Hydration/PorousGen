...
final_mask_dummy = (1 - (final_mask)).astype(np.uint8)
np.save("final_mask.npy", final_mask_dummy)
...
##########################################################
from helpers import save_all_slices

data = final_mask * 255
central_slice = data[data.shape[0]//2, :, :]

save_all_slices(data, VN=VN, VL=VL, output_directory=output_directory, base_name="3D")
